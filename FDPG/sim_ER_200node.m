...
G = gsp_erdos_renyi(N,0.1); % generating ER graph with p = 0.1
A1 = full(G.W);
% A1 = ones(N,N);
% A1 = full(A1);
...
h1 = figure(1);
hold on
p1 = plot(err_PG, '-.', 'LineWidth',1.0);
hold on
p2 = plot(err_FDPG, '-', 'LineWidth',1.0);
hold on
p3 = plot(err_kal, '--', 'LineWidth',1.0);
hold on
p4 = plot(err_ADMM, '-', 'LineWidth',1.0);
hold on
p5 = plot(err_DPG, '-.', 'LineWidth',1.0);
